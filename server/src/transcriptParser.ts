	...
	try {
		...
		if (record.type === 'assistant' && Array.isArray(record.message?.content)) {
			...
			if (hasToolUse) {
				...
				if (hasNonExemptTool) {
					startPermissionTimer(agentId, agents, permissionTimers, PERMISSION_EXEMPT_TOOLS, broadcaster);
				}
				...
				startWaitingTimer(agentId, TEXT_IDLE_DELAY_MS, agents, waitingTimers, broadcaster);
			}
		} else if (record.type === 'progress') {
			processProgressRecord(agentId, record, agents, waitingTimers, permissionTimers, broadcaster);
